...
export function initDb() {
  ...
  ensureColumn('dry_run_positions', 'partial_tp_stage', 'INTEGER DEFAULT 0');
  // FIX_CONFIG_PROPAGATION: 1 = the user manually overrode this position's TP/SL/trailing via
  // Telegram, so the engine pins the per-position snapshot for it. 0/null (default) = the position
  // follows the LIVE strategy config, so a config change/restore reaches already-open positions.
  ensureColumn('dry_run_positions', 'exit_overridden', 'INTEGER DEFAULT 0');
  // FIX_PNL_STUCK: persist the LAST observed (current) mcap/price each poll so the UI shows LIVE
  // PnL for open positions. Previously only high_water was stored, so display PnL used the peak —
  // losers read as 0% (high_water == entry) and winners froze at their high ("stuck"). The exit
  // engine always used current mcap correctly; only the DISPLAY drifted. Additive/nullable.
  ensureColumn('dry_run_positions', 'last_mcap', 'REAL');
  ensureColumn('dry_run_positions', 'last_price', 'REAL');
  ensureColumn('dry_run_positions', 'last_refreshed_at_ms', 'INTEGER');

  // PHASE_A3_PATCH: adversarial critique log
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_adversarial_created ON adversarial_critiques(created_at_ms DESC);
  `);
