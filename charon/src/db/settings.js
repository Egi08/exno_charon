...
// Pure, DB-free merge so a PARTIAL strategy update can NEVER wipe sibling keys.
// A full-replace here once clobbered the whole sniper config down to {max_hold_ms},
// reverting every exit + rug floor to code defaults. Unit-tested in
// test/strategyConfigMerge.test.js — keep it pure (no db access) so it stays testable.
export function mergeStrategyConfig(existing, patch) {
  let base = {};
...
export function updateStrategyConfig(id, config) {
  // MERGE over the existing config_json instead of full-replacing it (see mergeStrategyConfig).
  const row = db.prepare('SELECT config_json FROM strategies WHERE id = ?').get(id);
  const merged = mergeStrategyConfig(row && row.config_json, config);
