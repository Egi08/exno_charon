...
import { effectivePositionSize } from '../execution/sizing.js';

// AUDIT_PATCH: respect risk-watchdog HALT flag (advisory kill switch).
function isHalted() {
  try { return fs.existsSync(process.env.CHARON_HALT_FLAG || '/tmp/charon.HALT'); }
  catch { return false; }
}
...
export function canOpenMorePositions() {
  if (isHalted()) {
    console.log('[charon] HALT flag detected — refusing new positions. Remove /tmp/charon.HALT to resume.');
    return false;
  }
...
export function allPositions(limit = 10) {
  // DISPLAY_FILTER: only surface real dry_run/live positions. `pre_audit_archived` rows are
  // stale pre-audit test artifacts (old code + old tp80/sl30 config) — kept in the table for
  // audit history but never displayed. Matches the filter sizing.js (recent24hPnlPct) and
  // learning/summary.js already use, so display can't drift from accounting again.
  return db.prepare(`
...
export function createDryRunPosition(candidateId, candidate, decision, reason = 'llm_buy') {
  const strat = activeStrategy();
  // PHASE_A_PATCH: use confidence + drawdown-adaptive sizing instead of static base.
  // FIX_LIQUIDITY_SIZING: pass entry mcap so thin tokens get a smaller (lower-risk) size.
  const entryMcapForSizing = Number(candidate.metrics.marketCapUsd || candidate.metrics.graduatedMarketCapUsd || 0) || null;
  const sizing = effectivePositionSize(decision, { entryMcap: entryMcapForSizing });
  if (!sizing.sizeSol || sizing.sizeSol <= 0) {
    console.log(`[position] dry_run skip — ${sizing.reason}`);
    return null;
  }
  const sizeSol = sizing.sizeSol;
  console.log(`[position] dry_run size ${sizeSol} SOL — ${sizing.reason} (base=${sizing.baseSize})`);
...
export function createLivePosition(candidateId, candidate, decision, swap, reason = 'live_buy') {
  const strat = activeStrategy();
  // PHASE_A_PATCH: use confidence + drawdown-adaptive sizing instead of static base.
  const sizing = effectivePositionSize(decision);
  if (!sizing.sizeSol || sizing.sizeSol <= 0) {
    console.log(`[position] live skip — ${sizing.reason}`);
    return null;
  }
  const sizeSol = sizing.sizeSol;
  console.log(`[position] live size ${sizeSol} SOL — ${sizing.reason} (base=${sizing.baseSize})`);
  const entryPrice = Number(candidate.metrics.priceUsd || 0) || null;
  const entryMcap = Number(candidate.metrics.marketCapUsd || candidate.metrics.graduatedMarketCapUsd || 0) || null;
  const tp = Number(decision.suggested_tp_percent || strat.tp_percent || numSetting('default_tp_percent', 50));
  const sl = Number(decision.suggested_sl_percent || strat.sl_percent || numSetting('default_sl_percent', -25));
