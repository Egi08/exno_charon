...
import { effectivePositionSize } from './sizing.js';
...
export async function executeLiveBuy(selectedRow, decision, batchId, rows = [], triggerCandidateId = null) {
  ...
  // PHASE_A_PATCH: use confidence + drawdown-adaptive sizing.
  // FIX_LIQUIDITY_SIZING: pass entry mcap so thin tokens get a smaller (lower-risk) size.
  const entryMcapForSizing = Number(selectedRow.candidate?.metrics?.marketCapUsd || selectedRow.candidate?.metrics?.graduatedMarketCapUsd || 0) || null;
  const sizing = effectivePositionSize(decision, { entryMcap: entryMcapForSizing });
  if (!sizing.sizeSol || sizing.sizeSol <= 0) {
    throw new Error(`Sizing returned 0 — ${sizing.reason}`);
  }
  const amountLamports = Math.floor(sizing.sizeSol * 1_000_000_000);
  console.log(`[live_buy] size ${sizing.sizeSol} SOL — ${sizing.reason} (base=${sizing.baseSize})`);
...
export async function executeConfirmedIntent(chatId, intentId) {
  ...
  try {
    ...
    const strat = activeStrategy();
    // PHASE_A_PATCH: confidence + drawdown sizing for confirmed intents too.
    // FIX_LIQUIDITY_SIZING: pass entry mcap so thin tokens get a smaller (lower-risk) size.
    const entryMcapForSizing = Number(freshRow.candidate?.metrics?.marketCapUsd || freshRow.candidate?.metrics?.graduatedMarketCapUsd || 0) || null;
    const sizing = effectivePositionSize(decision, { entryMcap: entryMcapForSizing });
    if (!sizing.sizeSol || sizing.sizeSol <= 0) {
      db.prepare('UPDATE trade_intents SET status = ?, updated_at_ms = ? WHERE id = ?').run('rejected_sized_to_zero', now(), intentId);
      return bot.sendMessage(chatId, `Sized to 0 — ${sizing.reason}`, { parse_mode: 'HTML' });
    }
    const amountLamports = Math.floor(sizing.sizeSol * 1_000_000_000);
    const balance = await liveWalletBalanceLamports();
    if (balance < amountLamports + LIVE_MIN_SOL_RESERVE_LAMPORTS) {
