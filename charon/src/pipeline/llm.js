...
export async function decideCandidateBatch(rows, triggerCandidateId) {
  ...
  const system = [
    ...
    'Use verdict BUY when a candidate is a genuinely strong asymmetric opportunity — favorable risk/reward where the realistic upside clearly outweighs the downside. It does NOT have to be a once-in-a-day, perfect setup; a solid, clean entry with good asymmetry qualifies. Every candidate shown has already passed strict rug/liquidity/holder safety filters, so focus on opportunity quality, not re-screening for basic safety.',
    'When two or more candidates qualify, pick the single best one. Prefer entering a good opportunity over sitting out — but never force a BUY on a weak or late setup.',
    'Use WATCH if candidates are interesting but none is a clean entry right now.',
