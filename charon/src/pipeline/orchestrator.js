...
import { critiqueBuyDecision } from './adversarial.js';
...
export async function processCandidateFromSignals(signals) {
  ...
  if (selectedRow && boolSetting('agent_enabled', true) && batchDecision.verdict === 'BUY' && batchDecision.confidence >= numSetting('llm_min_confidence', 75)) {
    ...
    // PHASE_A3_PATCH: adversarial critique gate before buy.
    if (boolSetting('adversarial_critique_enabled', true)) {
      const critique = await critiqueBuyDecision(selectedRow, batchDecision);
      if (critique.abort) {
        console.log(`[adversarial] BLOCKED ${selectedRow.candidate.token.mint.slice(0,8)} — ${critique.reason}`);
        logDecisionEvent({
          batchId,
          triggerCandidateId: candidateId,
          selectedRow,
          rows,
          decision: batchDecision,
          action: 'entry_aborted_adversarial',
          guardrails: {
            adversarialMaxProb: critique.maxProbability,
            adversarialVerdict: critique.llmVerdict,
            scenariosCount: critique.scenarios?.length || 0,
          },
        });
        await sendTelegram([
          '🛡️ <b>Adversarial critic BLOCKED entry</b>',
          '',
          candidateSummary(selectedRow.candidate, batchDecision),
          '',
          `Reason: ${escapeHtml(critique.reason)}`,
          ...(critique.scenarios || []).slice(0, 3).map(s =>
            `• ${escapeHtml(s.scenario)} — ${s.probability}% (${s.severity})`),
        ].join('\n'));
        return;
      } else if (!critique.skipped) {
        console.log(`[adversarial] OK ${selectedRow.candidate.token.mint.slice(0,8)} — max scenario prob ${critique.maxProbability}%`);
      }
    }
...
export async function handleApprovedBuy(selectedRow, decision, batchId, rows = [], triggerCandidateId = null) {
  ...
  if (mode === 'dry_run') {
    ...
    if (!positionId) {
      // PHASE_A_PATCH: sizing returned 0 (confidence too low or HALT drawdown).
      logDecisionEvent({
        batchId,
        triggerCandidateId,
        selectedRow: freshSelectedRow,
        ...
        decision,
        mode,
        action: 'entry_sized_to_zero',
        guardrails: { confidence: decision.confidence, maxOpenPositions: numSetting('max_open_positions', 3) },
      });
      await sendTelegram([
        '⚠️ <b>Entry skipped — sized to 0</b>',
        '',
        candidateSummary(freshSelectedRow.candidate, decision),
        '',
        `Confidence ${decision.confidence}% or drawdown gate.`,
      ].join('\n'));
      return;
    }
    logDecisionEvent({
