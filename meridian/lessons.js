...
export async function recordPerformance(perf) {
  ...
  // In DRY_RUN, sim PnL is derived from bin-math price; a single-sided LP cannot
  // legitimately lose >90% within the SL window (SL fires ~-50%), so any -90%+ sim close
  // is a price-feed artifact (historical bin-math vs fromPricePerLamport mismatch) even
  // when mislabeled "stop loss (...)". Reject the whole band in sim so phantom losses can
  // never poison performance history / lessons / threshold evolution again.
  // In LIVE mode PnL comes from Meteora's own API, where a real rug CAN exceed -90%, so
  // keep the original narrower guard there (trust stop-loss-labeled disasters).
  const isDryRun = process.env.DRY_RUN === "true";
  const suspiciousAbsurdClosedPnl =
    Number.isFinite(pnl_pct) &&
    perf.initial_value_usd >= 20 &&
    pnl_pct <= -90 &&
    (isDryRun || !closeReasonText.includes("stop loss"));
