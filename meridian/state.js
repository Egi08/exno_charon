...
export function updatePnlAndCheckExits(position_address, positionData, mgmtConfig) {
  ...
  // ── Trailing TP (tightening, uncapped-runner) ──────────────────
  if (!pnl_pct_suspicious && pos.trailing_active) {
    const peak = pos.peak_pnl_pct;
    // Tightening trailing stop: give winners room early, protect MORE of the gain as the
    // peak grows. allowedDrop = max(base floor, peak × giveback fraction), where the
    // fraction shrinks as peak rises. No fixed TP cap — exit only on a peak-relative
    // pullback, so a runner can keep running (asymmetric upside).
    const baseDrop = mgmtConfig.trailingDropPct; // absolute floor for small peaks
    let givebackFrac;
    if (peak < 15) givebackFrac = 0.45;
    else if (peak < 30) givebackFrac = 0.35;
    else if (peak < 60) givebackFrac = 0.28;
    else givebackFrac = 0.22;
    const allowedDrop = Math.max(baseDrop, peak * givebackFrac);
    const dropFromPeak = peak - currentPnlPct;
    if (dropFromPeak >= allowedDrop) {
      return {
        action: "TRAILING_TP",
        reason: `Trailing TP: peak ${peak.toFixed(2)}% → current ${currentPnlPct.toFixed(2)}% (dropped ${dropFromPeak.toFixed(2)}% >= ${allowedDrop.toFixed(2)}% [${(givebackFrac * 100).toFixed(0)}% giveback])`,
