...
export async function getWalletBalances() {
  if (process.env.DRY_RUN === 'true' && process.env.SIMULATED_SOL_BALANCE) {
    const baseSol = Number(process.env.SIMULATED_SOL_BALANCE) || 1.0;
    let walletAddress;
    try { walletAddress = getWallet().publicKey.toString(); } catch { walletAddress = 'SIMULATED'; }
    // SIM_WALLET_FIX: derive live free-cash from sim positions instead of a frozen constant
    // (previous bug: "SOL tidak berubah" — balance stuck at 1.0 regardless of PnL).
    // free = base − locked principals + realized PnL. Falls back to base on any error.
    let st;
    try {
      const { computeSimWalletState } = await import('./sim-store.js');
      st = computeSimWalletState(baseSol);
    } catch (e) {
      log('wallet_warn', 'sim wallet state failed, using base: ' + e.message);
      st = { free_sol: baseSol, locked_sol: 0, realized_pnl_sol: 0, unrealized_fees_sol: 0, open_count: 0, closed_count: 0 };
    }
    const SOL_PRICE = 82.0;
    const freeSol = st.free_sol;
    const equitySol = baseSol + st.realized_pnl_sol + st.unrealized_fees_sol;
    return {
      wallet: walletAddress,
      sol: Math.round(freeSol * 1e6) / 1e6,          // free cash available to deploy
      sol_price: SOL_PRICE,
      sol_usd: Math.round(freeSol * SOL_PRICE * 100) / 100,
      usdc: 0,
      tokens: [],
      total_usd: Math.round(equitySol * SOL_PRICE * 100) / 100, // total equity (excl. open price PnL)
      // sim accounting detail
      locked_sol: st.locked_sol,
      realized_pnl_sol: st.realized_pnl_sol,
      unrealized_fees_sol: st.unrealized_fees_sol,
      total_equity_sol: Math.round(equitySol * 1e6) / 1e6,
      open_positions: st.open_count,
      simulated: true,
    };
  }
  let walletAddress;
