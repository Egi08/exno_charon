import * as simStore from "./sim-store.js"; // SIM_PATCH_APPLIED
import {
  Connection,
  Keypair,
  PublicKey,
...
// ─── Live pool vol/TVL from the WORKING discovery endpoint ─────
// FIX_FEES_ZERO: the sim fee model (sim-store.simAccrueFees) needs real 24h volume + TVL, but
// both deploy-enrichment and the poll loop previously fetched `dlmm.datapi.meteora.ag`, which is
// GEO-BLOCKED from this VPS → vol/TVL stayed 0 forever → fee accrual skipped every cycle → fees
// permanently $0. The screening source `pool-discovery-api.datapi.meteora.ag` IS reachable (it's
// how pools get discovered/selected in the first place), so reuse it. timeframe '24h' so `volume`
// is the true DAILY figure the fee model divides by 24*60 — 5m volume would under-report ~288x.
// Returns null fields on failure so callers HOLD the last known value instead of zeroing.
async function fetchLivePoolVolTvl(poolAddress) {
  try {
    const { getPoolDetail } = await import("./screening.js");
    const d = await getPoolDetail({ pool_address: poolAddress, timeframe: "24h" });
    if (!d) return null;
    const vol = Number(d.volume);
    const tvl = Number(d.active_tvl ?? d.tvl);
    return {
...
export async function deployPosition({
  ...
  if (process.env.DRY_RUN === "true") {
    const minBinIdSim = activeBin.binId - activeBinsBelow;
    const maxBinIdSim = (finalAmountX <= 0 && finalAmountY > 0) ? activeBin.binId : activeBin.binId + activeBinsAbove;
    const baseFactor = pool.lbPair.parameters?.baseFactor ?? 0;
    let baseFeePct = baseFactor > 0 ? (baseFactor * actualBinStep / 1e6 * 100) : 0.5;
    if (!Number.isFinite(baseFeePct) || baseFeePct <= 0 || baseFeePct > 5) baseFeePct = 0.5;
    let baseSym = 'TOKEN';
    try {
      const meta = await getPoolMetadata(pool_address);
      if (meta && meta.token_x_symbol) {
        baseSym = meta.token_x_symbol;
      }
    } catch (_) {}

    const positionAddress = simStore.simAddPosition({
      pool_address,
      ...
      lower_bin: minBinIdSim,
      upper_bin: maxBinIdSim,
      active_bin_at_deploy: activeBin.binId,
      active_price_at_deploy: Number(activeBin.price),
    ...
    // Enrich sim position with real-time pool data from the WORKING discovery endpoint
    // (dlmm.datapi.meteora.ag is geo-blocked from this VPS — see fetchLivePoolVolTvl).
    try {
      const live = await fetchLivePoolVolTvl(pool_address);
      if (live && (live.vol || live.tvl)) {
        const patch = {};
        if (live.vol) patch.pool_volume_24h_usd = live.vol;
        if (live.tvl) patch.pool_tvl_usd = live.tvl;
        simStore.simUpdate(positionAddress, patch);
        console.log(`[sim_deploy] enriched ${positionAddress.slice(0,8)} vol=${live.vol} tvl=${live.tvl}`);
      } else {
        console.log(`[sim_deploy] vol/tvl enrichment returned nothing for ${pool_address.slice(0,8)} — fees may under-report until first poll`);
      }
    } catch (e) { console.log("[sim_deploy] enrichment failed: " + e.message); }
    // SIM_PATCH: trackPosition so JS-coded trailing TP / OOR timer / pool-memory
    // snapshots run during dry-run (matches live behavior)
    try {
      const simNow = simStore.simGet(positionAddress);
      trackPosition({
        position: positionAddress,
        pool: pool_address,
        pool_name: pool_address.slice(0, 8),
        strategy: activeStrategy,
        bin_range: { min: minBinIdSim, max: maxBinIdSim },
        amount_sol: finalAmountY,
        amount_x: 0,
        ...
        fee_tvl_ratio: null,
        organic_score: null,
        initial_value_usd: finalAmountY * (simNow?.sol_price_at_deploy || 82),
        signal_snapshot: null,
      });
    } catch (e) { console.log("[sim_deploy] trackPosition failed: " + e.message); }
    return {
      dry_run: true,
      simulated: true,
      pool_address,
      position: positionAddress,
      txs: [`SIM_TX_${Date.now()}`],
      bin_step: actualBinStep,
      base_fee: baseFeePct,
      bins_below: activeBinsBelow,
      bins_above: activeBinsAbove,
      base_mint: pool.lbPair.tokenXMint.toString(),
      message: "DRY RUN — simulated position created in sim-store",
    };
  }
...
export async function getPositionPnl({ pool_address, position_address }) {
  if (process.env.DRY_RUN === "true") {
    const sim = simStore.simGet(position_address);
    if (sim) {
      const principalUsd = sim.amount_sol_y * (sim.sol_price_at_deploy || 82);
      const feesUsd = sim.accumulated_fees_usd;
      const totalClaimedUsd = sim.total_claimed_usd;
      return {
        simulated: true, position_address, pool: sim.pool,
        pnl_pct: 0, pnl_usd: feesUsd + totalClaimedUsd,
        unclaimed_fee_usd: feesUsd,
        all_time_fees_usd: feesUsd + totalClaimedUsd,
        current_value_usd: principalUsd + feesUsd,
        lower_bin: sim.lower_bin, upper_bin: sim.upper_bin, active_bin: sim.active_bin_at_deploy,
      };
    }
  }
  pool_address = normalizeMint(pool_address);
...
export async function getMyPositions({ force = false, silent = false, wallet_address = null } = {}) {
  if (process.env.DRY_RUN === "true") {
    const sims = simStore.simList({ openOnly: true });
    const positions = [];
    for (const p of sims) {
      let activeBinId = p.active_bin_at_deploy;
      let activePrice = p.active_price_at_deploy;
      try {
        const ab = await getActiveBin({ pool_address: p.pool });
        if (ab && Number.isFinite(Number(ab.binId))) {
          activeBinId = Number(ab.binId);
          // SIM_PRICE_FIX: deploy records active_price_at_deploy via getPriceOfBinByBinId
          // (raw bin-math). getActiveBin().price runs through fromPricePerLamport
          // (decimal-adjusted), differing by a constant ~decimal factor → constant
          // ~-99.9% phantom PnL unrelated to price movement. Use the SAME bin-math
          // function here so PnL only moves when the active bin actually moves.
          try {
            const { getPriceOfBinByBinId } = await getDLMM();
            const binStep = p.bin_step || 100;
            const mathPrice = Number(getPriceOfBinByBinId(activeBinId, binStep).toString());
            // ONLY bin-math price is comparable to active_price_at_deploy (also bin-math).
            // ab.price (fromPricePerLamport, decimal-adjusted) is NEVER comparable — using it
            // reintroduces the constant ~-99.9% phantom. On failure, hold activePrice at the
            // deploy price → neutral 0% PnL, never a false stop-loss. Do NOT fall back to ab.price.
            if (Number.isFinite(mathPrice) && mathPrice > 0) activePrice = mathPrice;
          } catch (e) { console.log("[sim_pos] bin-math price failed, holding entry price: " + e.message); }
        }
      } catch (e) { console.log("[sim_pos] getActiveBin failed: " + e.message); }
      let realVolumeUsd = p.pool_volume_24h_usd;
      let realTvlUsd = p.pool_tvl_usd;
      let poolApiOk = false;
      // Refresh vol/TVL live from the WORKING discovery endpoint every poll, so fees track the
      // pool's real-time activity (and a deploy-time volume spike decays instead of freezing).
      // dlmm.datapi.meteora.ag is geo-blocked here; pool-discovery-api.datapi.meteora.ag works.
      try {
        const live = await fetchLivePoolVolTvl(p.pool);
        if (live) {
          if (live.vol) realVolumeUsd = live.vol;
          if (live.tvl) realTvlUsd = live.tvl;
          // poolApiOk only when BOTH are usable — otherwise we hold stored and skip this cycle.
          poolApiOk = Boolean(live.vol && live.tvl);
          // Persist the refreshed values so a later transient failure still has recent data to hold.
          if (live.vol || live.tvl) {
            const patch = {};
            if (live.vol) patch.pool_volume_24h_usd = live.vol;
            if (live.tvl) patch.pool_tvl_usd = live.tvl;
            simStore.simUpdate(p.position, patch);
          }
        }
      } catch {}
      const volSafe = Number.isFinite(realVolumeUsd) && realVolumeUsd > 0 ? realVolumeUsd : 0;
      const tvlSafe = Number.isFinite(realTvlUsd) && realTvlUsd > 0 ? realTvlUsd : 0;
      if (volSafe === 0 || tvlSafe === 0) {
        // Skip fee accrual cycle when API failed or pool truly has zero vol/tvl.
        // Better to under-report than to invent fees from fake $5K vol / $20K tvl.
        simStore.simUpdate(p.position, { last_fee_accrual_at: Date.now() });
        if (!poolApiOk) console.log(`[sim_pos] pool API unreachable for ${p.pool.slice(0,8)} — fee accrual skipped this cycle`);
      } else {
        simStore.simAccrueFees(p.position, {
          active_bin_id: activeBinId,
          active_price: activePrice,
          volume_24h_usd: volSafe,
          tvl_usd: tvlSafe,
          sol_price_usd: p.sol_price_at_deploy || 82,
        });
      }
      const refreshed = simStore.simGet(p.position);
      const pnlPct = simStore.computePriceBasedPnlPct(refreshed, activePrice);
      const ageMin = Math.floor((Date.now() - refreshed.deployed_at) / 60000);
      const inRange = activeBinId >= refreshed.lower_bin && activeBinId <= refreshed.upper_bin;
      // SIM_SANITY (defense-in-depth vs the phantom -99.9% class of bug): a single-sided DLMM LP
      // that is still IN RANGE cannot legitimately be down >90%. If it is, the price feed/unit is
      // broken. Flag it so the exit logic (state.js trailing/peak skip + index.js stop-loss guard)
      // ignores the bad tick instead of phantom-closing. A real catastrophic dump pushes the active
      // bin OUT of range, so this never false-positives on a genuine loss.
      const pnlSuspicious = inRange && Number.isFinite(pnlPct) && pnlPct <= -90;
      if (pnlSuspicious) {
        log("positions_warn", `[SIM_SANITY] In-range PnL ${pnlPct.toFixed(2)}% for ${String(refreshed.position).slice(0,8)} (active bin ${activeBinId} within [${refreshed.lower_bin},${refreshed.upper_bin}]) — price-unit regression? Ignoring tick.`);
      }
      const solPriceUsd = refreshed.sol_price_at_deploy || 82;
      const principalSol = refreshed.amount_sol_y;
      const principalUsd = principalSol * solPriceUsd;
      const pnlPriceSol = principalSol * (pnlPct / 100);
      const pnlPriceUsd = principalUsd * (pnlPct / 100);
      const unclaimedSol = refreshed.accumulated_fees_sol;
      const unclaimedUsd = refreshed.accumulated_fees_usd;
      const totalValueSol = principalSol + pnlPriceSol + unclaimedSol;
      const totalValueUsd = principalUsd + pnlPriceUsd + unclaimedUsd;
      const totalPnlSol = pnlPriceSol + unclaimedSol + refreshed.total_claimed_sol;
      const totalPnlUsd = pnlPriceUsd + unclaimedUsd + refreshed.total_claimed_usd;
      const pairName = refreshed.base_symbol && refreshed.base_symbol !== 'TOKEN'
        ? `${refreshed.base_symbol}-${refreshed.quote_symbol || 'SOL'}`
        : `SIM-${refreshed.pool.slice(0,4)}`;
      // DASHBOARD_SNAPSHOT: persist the live mark-to-market view to sim-positions.json so the
      // file-based PnL dashboard reads the SAME numbers as /positions (single source of truth).
      // Without this the dashboard can only see accumulated_fees_usd (fees), not price-PnL —
      // which is why it showed +$0.02 while /positions showed PnL +$1.4 for the same position.
      try {
        simStore.simUpdate(refreshed.position, {
          live_pair:        pairName,
          live_pnl_usd:     Number(totalPnlUsd.toFixed(2)),
          live_pnl_pct:     Number(pnlPct.toFixed(2)),
          live_value_usd:   Number(totalValueUsd.toFixed(2)),
          live_in_range:    inRange,
          live_age_min:     ageMin,
          live_pnl_suspicious: pnlSuspicious,
          live_snapshot_at: Date.now(),
        });
      } catch (e) { /* snapshot is best-effort; never block position reporting */ }
      positions.push({
        position: refreshed.position,
        pool: refreshed.pool,
        pool_name: refreshed.pool_name,
        pair: pairName,
        base_mint: refreshed.base_mint,
        base_symbol: refreshed.base_symbol,
        quote_mint: refreshed.quote_mint,
        quote_symbol: refreshed.quote_symbol,
        lower_bin: refreshed.lower_bin,
        upper_bin: refreshed.upper_bin,
        active_bin: activeBinId,
        in_range: inRange,
        out_of_range: !inRange,
        minutes_out_of_range: 0,
        age_minutes: ageMin,
        amount_sol: principalSol,
        pnl_pct: Number(pnlPct.toFixed(2)),
        pnl_sol: Number(totalPnlSol.toFixed(4)),
        pnl_usd: Number(totalPnlUsd.toFixed(2)),
        unclaimed_fees_sol: Number(unclaimedSol.toFixed(6)),
        unclaimed_fees_usd: Number(unclaimedUsd.toFixed(2)),
        fees_earned_sol: Number(unclaimedSol.toFixed(6)),
        fees_earned_usd: Number(unclaimedUsd.toFixed(2)),
        total_claimed_sol: Number(refreshed.total_claimed_sol.toFixed(6)),
        total_claimed_usd: Number(refreshed.total_claimed_usd.toFixed(2)),
        total_value_sol: Number(totalValueSol.toFixed(4)),
        total_value_usd: Number(totalValueUsd.toFixed(2)),
        fee_per_tvl_24h: 0,
        pnl_pct_suspicious: pnlSuspicious,
        deployed_at: new Date(refreshed.deployed_at).toISOString(),
        simulated: true,
      });
    }
    return { wallet: 'SIMULATED', total_positions: positions.length, positions };
  }
...
export async function claimFees({ position_address }) {
  position_address = normalizeMint(position_address);
  if (process.env.DRY_RUN === "true") {
    const result = simStore.simClaim(position_address);
    if (!result) return { dry_run: true, success: false, error: "Sim position not found" };
    return {
      dry_run: true, simulated: true, success: true,
      position: position_address,
      txs: [`SIM_CLAIM_${Date.now()}`],
      claimed_sol: result.claimed_sol,
      claimed_usd: result.claimed_usd,
      base_mint: simStore.simGet(position_address)?.base_mint,
    };
  }
...
export async function closePosition({ position_address, reason }) {
  position_address = normalizeMint(position_address);
  if (process.env.DRY_RUN === "true") {
    const before = simStore.simGet(position_address);
    if (!before) return { dry_run: true, success: false, error: "Sim position not found" };
    let currentPrice = before.active_price_at_deploy;
    try {
      const ab = await getActiveBin({ pool_address: before.pool });
      if (ab && Number.isFinite(Number(ab.binId))) {
        // SIM_PRICE_FIX: match deploy-side pricing (getPriceOfBinByBinId raw bin-math)
        // instead of getActiveBin().price (fromPricePerLamport decimal-adjusted), which
        // differs by a constant decimal factor and produced a constant ~-99.9% phantom PnL.
        try {
          const { getPriceOfBinByBinId } = await getDLMM();
          const binStep = before.bin_step || 100;
          const mathPrice = Number(getPriceOfBinByBinId(Number(ab.binId), binStep).toString());
          // ONLY bin-math is comparable to active_price_at_deploy. NEVER fall back to ab.price
          // (decimal-adjusted) — that reintroduces the phantom -99.9%. On failure hold the
          // entry price (currentPrice already initialized to it) → 0% PnL, conservative.
          if (Number.isFinite(mathPrice) && mathPrice > 0) currentPrice = mathPrice;
        } catch (e) { console.log("[sim_close] bin-math price failed, holding entry price: " + e.message); }
      }
    } catch (e) { console.log("[sim_close] getActiveBin failed: " + e.message); }
    simStore.simClose(position_address, reason);
    const finalFeesSol = before.accumulated_fees_sol + before.total_claimed_sol;
    const finalFeesUsd = before.accumulated_fees_usd + before.total_claimed_usd;
    const pnlPct = before.active_price_at_deploy > 0
      ? ((currentPrice / before.active_price_at_deploy) - 1) * 100 : 0;
    const pnlSol = before.amount_sol_y * (pnlPct / 100) + finalFeesSol;
    const pnlUsd = pnlSol * (before.sol_price_at_deploy || 82);
    // SIM_WALLET_FIX: persist realized PnL on the closed sim position so getWalletBalances
    // can reflect it. Previously the dry-run wallet was a frozen constant ("SOL tidak berubah").
    // Free-cash accounting: while open, principal (amount_sol_y) is locked; on close the
    // principal returns and realized_pnl_sol (price PnL + fees) is added to free cash.
    try {
      simStore.simUpdate(position_address, {
        realized_pnl_sol: pnlSol,
        realized_pnl_usd: pnlUsd,
        closed_pnl_pct: pnlPct,
      });
    } catch (e) { console.log("[sim_close] persist realized pnl failed: " + e.message); }
    // SIM_PATCH: recordPerformance so lessons.js learns from sim closes.
    // Built-in guards in recordPerformance reject suspicious unit-mixed records,
    // and -90%+ closes without "stop loss" reason — that's intended.
    try {
      const principalUsd = before.amount_sol_y * (before.sol_price_at_deploy || 82);
      const finalValueUsd = principalUsd * (1 + pnlPct / 100);
      const minutesHeld = Math.max(1, Math.floor((Date.now() - before.deployed_at) / 60000));
      await recordPerformance({
        position: position_address,
        pool: before.pool,
        pool_name: before.pool_name,
        base_mint: before.base_mint,
        strategy: before.strategy,
        amount_sol: before.amount_sol_y,
        initial_value_usd: principalUsd,
        final_value_usd: finalValueUsd,
        fees_earned_sol: finalFeesSol,
        fees_earned_usd: finalFeesUsd,
        minutes_held: minutesHeld,
        minutes_in_range: minutesHeld, // sim approximation; tighten later if needed
        close_reason: reason || "sim_close",
        deployed_at: new Date(before.deployed_at).toISOString(),
        volatility: null,
      });
    } catch (e) { console.log("[sim_close] recordPerformance failed: " + e.message); }
    return {
      dry_run: true, simulated: true, success: true,
      position: position_address,
      pool: before.pool,
      base_mint: before.base_mint,
      txs: [`SIM_CLOSE_${Date.now()}`],
      pnl_pct: pnlPct,
      pnl_sol: pnlSol,
      pnl_usd: pnlUsd,
      fees_earned_sol: finalFeesSol,
