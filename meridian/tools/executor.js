...
async function validateDeployPoolThresholds(args) {
  ...
  if (minFeeActiveTvlRatio != null && minFeeActiveTvlRatio > 0) {
    // FIX_5M_PHANTOM_ZERO: the deploy gate re-fetches pool detail at the screening timeframe
    // (default 5m). The 5m fee/active-TVL ratio is extremely noisy and frequently reads EXACTLY 0
    // when no swap hit the active bin in the last 5 minutes — even for perfectly healthy pools
    // (proven live: SPCX 5m=0.0093 vs 24h=1.70; the value can momentarily hit 0). Gating a deploy
    // on that instantaneous value phantom-blocks good entries every cycle. Re-check on the STABLE
    // 24h window before blocking; only block when BOTH the short window AND 24h are below threshold
    // (a genuinely dead pool fails both, a merely-quiet-right-now pool passes on 24h).
    let effectiveRatio = feeActiveTvlRatio;
    if (effectiveRatio == null || effectiveRatio < minFeeActiveTvlRatio) {
      try {
        const detail24 = await fetchFreshPoolDetail(args.pool_address, "24h");
        const ratio24 = poolDetailFeeActiveTvlRatio(detail24);
        if (ratio24 != null) effectiveRatio = Math.max(effectiveRatio ?? 0, ratio24);
      } catch (_) {
        // 24h fetch failed — keep the 5m value and fall through to the block below.
      }
    }
    if (effectiveRatio == null || effectiveRatio < minFeeActiveTvlRatio) {
      return {
        pass: false,
        reason: `Pool fee/active-TVL ${effectiveRatio ?? "unknown"} is below configured minFeeActiveTvlRatio ${minFeeActiveTvlRatio} (checked 5m + 24h).`,
      };
    }
  }
...
async function runSafetyChecks(name, args) {
  // AUDIT_PATCH: respect risk-watchdog HALT flag (advisory kill switch)
  if (name === "deploy_position") {
    try {
      const fs = await import("fs");
      const flag = process.env.MERIDIAN_HALT_FLAG || "/tmp/meridian.HALT";
      if (fs.existsSync(flag)) {
        return { pass: false, reason: `HALT flag present at ${flag}. Remove the flag to resume deploys.` };
      }
    } catch {}
  }
  switch (name) {
