...
export const config = {
  ...
  // ─── HiveMind ─────────────────────────
  hiveMind: {
    // Explicit kill switch: when hiveMindEnabled is false, HiveMind is fully off regardless of
    // the hardcoded default url/apiKey fallbacks below (which otherwise make it look "on" and
    // spam "Invalid HiveMind API key" on every heartbeat). Defaults to true to preserve prior behavior.
...
export function computeDeployAmount(walletSol) {
  ...
  // FIX_DEPLOY_FLOOR_ROUNDING: toFixed(2) can round a >2-decimal floor (e.g. deployAmountSol
  // 0.2037) DOWN to 0.20 — which the executor's own minDeploy guard (=deployAmountSol) then
  // REJECTS, wasting the first deploy_position attempt every screening cycle. Never return
  // below the configured floor.
  return rounded >= floor ? rounded : parseFloat(floor.toFixed(4));
}
