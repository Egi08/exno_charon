...
// FIX_UA_REQUIRED: ai.masanto.id sits behind Cloudflare Bot-Fight Mode which 403s
// (error code 1010 / "Your request was blocked") on non-browser client signatures —
// the OpenAI-Node SDK's default User-Agent ("OpenAI/NodeJS/x") is banned. A browser
// User-Agent passes the challenge. Override via env LLM_USER_AGENT if the endpoint changes.
...
const client = new OpenAI({
  ...
  timeout: 5 * 60 * 1000,
  defaultHeaders: { "User-Agent": LLM_USER_AGENT },
});

const DEFAULT_MODEL = process.env.LLM_MODEL || "openrouter/healer-alpha";

// FIX_STREAM_REQUIRED: commandcode (and some OpenAI-compatible routers, incl. the 9router
// upstream) REJECT a non-streamed request when `tools` are present — it 400s with
// "Invalid input: expected true at \"params.stream\"". The fix is to ALWAYS stream and
// reassemble the chunks back into the exact non-streaming response shape the ReAct loop
// below depends on ({ choices: [{ message:{ role, content, tool_calls }, finish_reason }] }).
// Transient 502/503/529 are surfaced as an error body (NOT thrown) so the existing inline
// retry + fallback-model logic keeps working unchanged; 400-class errors (system role,
// tool_choice) are re-thrown so their dedicated handlers still fire.
function isTransientStatus(s) { return s === 502 || s === 503 || s === 529; }
async function createChatCompletion(reqParams) {
  let stream;
  try {
    stream = await client.chat.completions.create({ ...reqParams, stream: true });
  } catch (error) {
    const status = error?.status ?? error?.code;
    if (isTransientStatus(status)) {
      return { choices: [], error: { code: status, message: String(error?.message || error) } };
    }
    throw error;
  }

  const acc = { role: "assistant", content: "" };
    ...
    for await (const chunk of stream) {
      ...
      if (delta.tool_calls) {
        for (const tcd of delta.tool_calls) {
          ...
          if (!e) { e = { id: tcd.id || "", type: tcd.type || "function", function: { name: "", arguments: "" } }; toolMap.set(idx, e); }
          if (tcd.id) e.id = tcd.id;
          if (tcd.type) e.type = tcd.type;
          if (tcd.function?.name) e.function.name += tcd.function.name;
          if (tcd.function?.arguments) e.function.arguments += tcd.function.arguments;
        }
      }
  ...
  const tool_calls = [...toolMap.entries()].sort((a, b) => a[0] - b[0]).map(([, v]) => v);
  const message = { role: acc.role || "assistant", content: acc.content || null };
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  // FIX_BLOCKED_DEPLOY_LOCK: a deploy that was BLOCKED by a safety check moved no capital, so the
  // model must be free to try a DIFFERENT candidate pool. Only a deploy that actually executed
  // (or errored mid-flight) gets locked to prevent double-deploying. Bound blocked retries so a
  // bad screening cycle can't spam deploy attempts indefinitely.
  let deployBlockedAttempts = 0;
  const MAX_DEPLOY_BLOCKED_ATTEMPTS = 4;
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      // FIX_FALLBACK_MODEL: the previous default "stepfun/step-3.5-flash:free" is an OpenRouter
      // model id that does NOT exist on the commandcode/9router endpoint — switching to it on a
      // transient error would fail the retry exactly when it's needed. Use a model that is
      // guaranteed present on the active endpoint (deepseek-v4-flash: cheap+fast sibling), and
      // allow an env override so the fallback tracks whatever endpoint the bot points at.
      const FALLBACK_MODEL = process.env.LLM_FALLBACK_MODEL || "cmc/deepseek/deepseek-v4-flash";
      let response;
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          response = await createChatCompletion(reqParams);
        } catch (error) {
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        // FIX_BLOCKED_DEPLOY_LOCK: only lock deploy_position once it actually MOVED CAPITAL.
        // A safety-block (result.blocked) deployed nothing, so the model is free to try a DIFFERENT
        // pool — this is what unsticks the "block first pick → duplicate-blocked → NO DEPLOY" loop.
        // Bound blocked attempts so a bad cycle can't spam deploys; a non-blocked deploy (executed
        // or errored mid-flight) locks immediately so we never double-deploy real capital.
        // For close/swap: only lock on success so genuine failures can be retried.
        if (functionName === "deploy_position") {
          if (result?.blocked === true) {
            deployBlockedAttempts += 1;
            if (deployBlockedAttempts >= MAX_DEPLOY_BLOCKED_ATTEMPTS) {
              firedOnce.add(functionName);
              log("agent", `deploy_position blocked ${deployBlockedAttempts}x this session — locking to stop retry spam`);
            }
          } else {
            firedOnce.add(functionName);
          }
        } else if (NO_RETRY_TOOLS.has(functionName)) {
          firedOnce.add(functionName);
