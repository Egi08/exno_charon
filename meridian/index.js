...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // ── Deterministic execution: hard CLOSES run in JS, NOT via LLM ──
    // Stop-loss / trailing / OOR / low-yield closes are SAFETY-CRITICAL and must fire even
    // when the LLM router is down (502/500). Previously every action was executed via
    // agentLoop(), so an LLM outage during a market dump would leave positions open past
    // their stop loss (the cycle threw and was caught at the bottom, closing nothing).
    // Execute CLOSE directly here; the LLM only handles CLAIM/INSTRUCTION (non-critical).
    const closeActions = [];
    const llmActions = [];
    for (const p of positionData) {
      const a = actionMap.get(p.position);
      if (a.action === "CLOSE") closeActions.push(p);
      else if (a.action === "CLAIM" || a.action === "INSTRUCTION") llmActions.push(p);
    }

    if (closeActions.length > 0) {
      log("cron", `Management: ${closeActions.length} deterministic CLOSE(s) — executing in JS (LLM-independent)`);
      const closeResults = [];
      for (const p of closeActions) {
        const a = actionMap.get(p.position);
        try {
          const result = await closePosition({ position_address: p.position, reason: a.reason });
          if (result?.success) {
            closeResults.push(`${p.pair}: ✅ closed — ${a.reason}`);
            log("cron", `[deterministic close] ${p.pair} closed: ${a.reason}`);
          } else {
            closeResults.push(`${p.pair}: ⚠️ close FAILED (${result?.error || "unknown"})`);
            log("cron_error", `[deterministic close] ${p.pair} failed: ${result?.error || "unknown"}`);
          }
        } catch (e) {
          closeResults.push(`${p.pair}: ⚠️ close ERROR (${e.message})`);
          log("cron_error", `[deterministic close] ${p.pair} threw: ${e.message}`);
        }
      }
      mgmtReport += `\n\n${closeResults.join("\n")}`;
    }

    // ── Call LLM only for CLAIM / INSTRUCTION (non-safety-critical) ──
    if (llmActions.length > 0) {
      log("cron", `Management: ${llmActions.length} claim/instruction action(s) — invoking LLM [model: ${config.llm.managementModel}]`);
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  // Uncapped-runner: when trailing TP is enabled it manages exits dynamically (tightening
  // giveback as the peak grows), so a fixed hard TP cap would defeat letting winners run.
  // Only apply the fixed cap when trailing is OFF.
